...
describe('Test EducationList component', () => {
  const diseases = [
    {
      url:
      ...
      institution: {
        ...
        patients_url:
          'https://127.0.0.1:8000/api/institutions/cuf/patients',
        followers_url:
      ...
      education_urls: {
        article_endpoint:
          'https://diabetes-cuf.cms.labs.promptly.health/api/v2/',
        sidebar_endpoint: '',
        articles_endpoint:
  ...
  it('Should render nothing to read because no diseases no articles', () => {
    const {getByTestId} = render(
      <ApplicationProvider mapping={mapping} theme={light}>
        <EducationList diseases={[]} articles={{data: {}}} />
      </ApplicationProvider>,
    );
  ...
  it('Should render nothing to read because no articles', () => {
    const {getByTestId} = render(
      <ApplicationProvider mapping={mapping} theme={light}>
        <EducationList diseases={diseases} articles={{data: {}}} />
      </ApplicationProvider>,
    );
    const element = getByTestId('statusBannerTitle');
    expect(element.props.children).toBe('Nothing to read yet!');
  ...
  it('Should render Improve your health', () => {
    const {getByTestId} = render(
      <ApplicationProvider mapping={mapping} theme={light}>
        <EducationList diseases={diseases} articles={articles} />
      </ApplicationProvider>,
    );
    const element = getByTestId('statusBannerTitle');
    expect(element.props.children).toBe('Improve your health');
  });
});

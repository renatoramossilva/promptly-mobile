...
import EducationPreviewItem from '../Education.Preview.Item';
import {ApplicationProvider} from '@ui-kitten/components';
import {mapping, light} from '@eva-design/eva';

describe('Test EducationPreviewItem component', () => {
  const article = {
    id: 166,
  ...
  it('Should render correct title', () => {
    const {getByTestId} = render(
      <ApplicationProvider mapping={mapping} theme={light}>
        <EducationPreviewItem article={article} />
      </ApplicationProvider>,
    );
  ...
  it('Should render correct image', () => {
    ...
    expect(element.props.source.uri).toBe(article.previewImage);
  });
});
